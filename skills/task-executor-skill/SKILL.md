...
description: 任务执行者 Skill — 通过 CLI 工具领取子任务、提交成果、处理返工、上报跨岗需求
---
...
1. 获取规则 → 2. 读自省笔记 → 3. 检查积分 → 4. 查看我的子任务 → 5. 了解上下文 → 6. 开始执行 → 7. 完成后提交 → 8. 如有返工，查看审查记录后修复再提交
...
当你在执行任务过程中发现需要其他岗位协助时，使用 `escalate` 命令正式上报给 Planner：

```bash
escalate <task_id> "上报标题" --desc "详细描述需要什么帮助" --category <建议类别> --sub-task-id <当前子任务ID>
```
...
| category | 适用场景 |
|----------|---------|
| `product` | 需要产品经理补充需求、确认业务逻辑 |
| `ui_design` | 需要UI设计师补充设计稿、确认样式 |
| `frontend` | 需要前端开发协助页面实现 |
| `backend` | 需要后端开发提供API接口 |
| `database` | 需要数据库开发提供DDL脚本 |
| `devops` | 需要运维协助部署或环境配置 |

**示例**：
```bash
# 产品经理发现需要UI设计
escalate <task_id> "首页UI设计" --desc "需要设计首页布局" --category ui_design --sub-task-id <当前子任务ID>

# 前端发现缺少后端API
escalate <task_id> "用户登录API" --desc "需要后端提供登录接口" --category backend --sub-task-id <当前子任务ID>
```

> ⚠️ **重要**：需要其他岗位协助时必须用 `escalate` 命令正式上报，不要只写在日志里（会被遗漏）！
...
log create "reflection" "子任务xxx被扣分：原因。改进：怎么避免" --sub-task-id <id>
...
log list --action plan --days 7 --limit 30  # 搜索排障方案
...
- 每次唤醒先读 `log mine --action reflection` 回顾自省笔记
- 每次唤醒时检查 `score logs`，有扣分则查看 `review list` 分析原因并写自省笔记
- 所有产出物必须放在子任务对应的工作目录下
- 提交前先写交付摘要（`log create "delivery" ...`），再 `st submit`
- 遇到问题先查日志找已有方案，自己尝试解决，实在不行再用 `escalate` 上报
